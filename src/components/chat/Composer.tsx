...
export function Composer({ modelId, onModelChange, isGenerating, sendOnEnter, onSend, onStop }: Props) {
  ...
  }, [text, attachments, isGenerating, onSend, resize]);
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 pt-2 pb-[max(1rem,env(safe-area-inset-bottom))] sm:pb-[max(1.5rem,env(safe-area-inset-bottom))]">
      ...
      >
        ...
        <div className="flex items-end gap-2 px-3 py-3">
          ...
          <textarea
            ...
            rows={1}
            placeholder="Ask MAAR anything…"
            aria-label="Message MAAR"
          ...
          {isGenerating ? (
            ...
            <button
              ...
              className="mb-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gold text-[#12100A] transition-transform hover:brightness-95 disabled:opacity-30 disabled:hover:brightness-100"
      ...
      <p className="mt-2 text-center text-[11px] text-ink-faint">
        {sendOnEnter ? 'Enter to send · Shift+Enter for a new line' : 'Shift+Enter or the button to send'} · Conversations
        stay on this device
      </p>
    </div>
  );
